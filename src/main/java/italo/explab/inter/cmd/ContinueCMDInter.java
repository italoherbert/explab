...
public class ContinueCMDInter extends Inter {
    ...
    public InterResult interpreta( ExecNo no, ExecNo base, InterAplic aplic, Codigo codigo, InterTO to, int i, int i2 ) {
        ...
        int cont = contUtil.contaTextoValor( codigo, i, PalavrasReservadas.CONTINUE );
        if ( cont == 0 )
        ...
        Continue ct = aplic.getExecutor().getExecManager().getExecNoFactory().getCMDFactory().novoContinue( i, no, codigo );
                                
        return new InterResult( ct, j ); 
    }
