...
public class SuperChamadaConstrutorInter extends Inter {
    ...
    public InterResult interpreta( ExecNo no, ExecNo base, InterAplic aplic, Codigo codigo, InterTO to, int i, int i2 ) {
        ...
        AnaliseResult aresult = asManager.getSuperChamadaConstrutorAnalisador().analisa( codigo, i );
        if ( aresult.getJ() == 0 )
        ...
        EsteOuSuperConstrutor chamadaConstrutorInst = aplic.getExecutor().getExecManager().getExecNoFactory().novoSuperConstrutor( i, no, codigo );
                        
        int j = 0;
        j += contUtil.contaTextoValor( codigo, i+j, PalavrasReservadas.SUPER );
