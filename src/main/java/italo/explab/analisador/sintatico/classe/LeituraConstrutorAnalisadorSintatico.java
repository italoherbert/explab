...
public class LeituraConstrutorAnalisadorSintatico implements AnalisadorSintatico {

    private final AnalisadorSintaticoManager manager;

    public LeituraConstrutorAnalisadorSintatico( AnalisadorSintaticoManager manager ) {
    ...
    public AnaliseResult analisa( Codigo codigo, int i ) {        
        ...
        int cont = manager.getContUtil().contaTextoValor( codigo, i+j, PalavrasReservadas.CONSTRUTOR );
        if ( cont == 0 )
