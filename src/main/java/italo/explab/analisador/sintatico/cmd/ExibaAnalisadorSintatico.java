...
public class ExibaAnalisadorSintatico implements AnalisadorSintatico {

    private final AnalisadorSintaticoManager manager;

    public ExibaAnalisadorSintatico( AnalisadorSintaticoManager manager ) {
    ...
    public AnaliseResult analisa( Codigo codigo, int i ) {
        ...
        int cont = manager.getContUtil().contaTextoValor( codigo, i+j, PalavrasReservadas.EXIBA );
        if ( cont == 0 )
